...
video_path = "videos/red_light_4.mp4"
...
while True:
    ...
    for track in tracks:
        ...
        if int(track_id) in redlight_run:
            ...
            tempStr = "Violating Car ID: " + str(int(track_id)) + "   Frame Number: " + str(current_frame[count]) + "   Violation Type: Running Red Light\n"
...
for frame in current_frame:
    extractFrame(video_path, frame)
    root = Tk()
    image = ImageTk.PhotoImage(Image.open(r'images/frame%d.jpg' % frame))  
    Label(root, text=Lines[count].strip(), font=('Times', 24), image=image, compound='bottom').pack()
    count += 1
    root.mainloop()